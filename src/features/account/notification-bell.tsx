...
import { realtime, leaveRealtime } from "@/shared/lib/realtime/echo";
...
export function NotificationBell() {
  ...
  useEffect(() => { void apiClient.request<{ data: { data: Item[] }; unread_count: number }>(`${routes.api.notifications}?unread=1`).then((response) => { setItems(response.data.data.slice(0, 10)); setUnread(response.unread_count); }).catch(() => undefined); }, []);
  useEffect(() => { if (!user) return; const channel = `users.${user.id}`; const inbox = realtime()?.private(channel); inbox?.listen(".notification.created", ({ notification }: { notification: Item }) => { setItems((current) => [notification, ...current.filter((item) => item.id !== notification.id)].slice(0, 10)); setUnread((count) => notification.read_at ? count : count + 1); window.dispatchEvent(new CustomEvent("malik:notification", { detail: notification })); }); inbox?.listen(".message.created", () => window.dispatchEvent(new Event("malik:message"))); return () => leaveRealtime(channel); }, [user]);
