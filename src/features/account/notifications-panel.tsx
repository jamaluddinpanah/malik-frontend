...
export function NotificationsPanel() {
  ...
  return <section className="settings-panel notifications-panel">{notice ? <Toast title={notice.title} message={notice.message} tone={notice.tone} onDismiss={() => setNotice(null)} /> : null}<header><div><span className="notifications-eyebrow"><Bell size={14} /> Account updates</span><h1>Notifications</h1><p>Stay up to date with your listings, messages, and account activity.</p></div><Button variant="ghost" type="button" onClick={() => readAll()}><CheckCheck size={15} /> Mark all as read</Button></header><label className="notification-preference"><span><b>Message notifications</b><small>Receive new message alerts in your account.</small></span><input type="checkbox" checked={messageNotifications} onChange={(event) => void toggleMessageNotifications(event.target.checked)} /></label>{loading ? <p className="notification-state" role="status">Loading notifications...</p> : error ? <p className="notification-state" role="alert">Notifications could not be loaded.</p> : !items.length ? <div className="notification-empty"><Bell size={28} /><b>No notifications yet</b><p>New updates about your account will appear here.</p></div> : <><div className="notification-list">{items.map((item) => <Button variant="ghost" type="button" className={item.read_at ? "notification-item" : "notification-item unread"} key={item.id} onClick={() => markRead(item.id)}><span className="notification-icon"><Bell size={16} /></span><span className="notification-copy"><b>{item.data?.title ?? "Notification"}</b><small>{item.data?.message ?? ""}</small><time dateTime={item.created_at}>{new Date(item.created_at).toLocaleString()}</time></span>{!item.read_at ? <i aria-label="Unread" /> : null}</Button>)}</div>{nextCursor || hasExpanded ? <div className="notification-pagination">{hasExpanded ? <button type="button" onClick={showLess}>Show less</button> : null}{nextCursor ? <button type="button" disabled={loadingMore} onClick={() => { setLoadingMore(true); void load(nextCursor); }}>{loadingMore ? "Loading..." : "Show more"}</button> : null}</div> : null}</>}</section>;
}
