...
export function LocationSelector({ value, onChange }: { value?: number; onChange: (area: LocationOption) => void }) {
  ...
  const props = { className: styles.select, classNamePrefix: "location-select", isRtl: locale !== "en", isSearchable: true, menuPortalTarget: typeof document === "undefined" ? undefined : document.body, styles: { menuPortal: (base: object) => ({ ...base, zIndex: 20 }) } };
  ...
  return <div className={styles.selector}>
    <ReactSelect {...props} options={optionsFor(roots)} value={selected(country)} aria-label={t("selectLocation")} placeholder={t("selectLocation")} onChange={(option) => { if (!option) return; resolvedValue.current = option.area.id; setCountry(option.area); resetBelowCountry(); onChange(option.area); }} />
    {country ? <ReactSelect {...props} options={optionsFor(provinces)} value={selected(province)} aria-label={t("selectProvince")} placeholder={t("selectProvince")} noOptionsMessage={() => t("selectProvince")} onChange={(option) => { if (!option) return; resolvedValue.current = option.area.id; setProvince(option.area); setDistrict(null); setVillage(null); setDistricts([]); setProvinceVillages([]); setVillages([]); onChange(option.area); }} /> : null}
    {districtOptions.length ? <ReactSelect {...props} options={optionsFor(districtOptions)} value={selected(district)} aria-label={t("selectDistrict")} placeholder={t("selectDistrict")} noOptionsMessage={() => t("selectDistrict")} onChange={(option) => { if (!option) return; resolvedValue.current = option.area.id; setDistrict(option.area); setVillage(null); setVillages([]); onChange(option.area); }} /> : null}
    {district?.children_count ? <ReactSelect {...props} options={optionsFor(villages)} value={selected(village)} aria-label={t("selectVillage")} placeholder={t("selectVillage")} noOptionsMessage={() => t("selectVillage")} onChange={(option) => { if (!option) return; resolvedValue.current = option.area.id; setVillage(option.area); onChange(option.area); }} /> : null}
  </div>;
}