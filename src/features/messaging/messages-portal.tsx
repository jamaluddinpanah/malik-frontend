"use client";
import { Textarea, Input } from "@/shared/ui/form-controls";
...
export function MessagesPortal() {
  ...
  return <section className="messages-portal">{notice ? <Toast title={notice.title} message={notice.message} tone={notice.tone} onDismiss={() => setNotice(null)} /> : null}{pendingAction ? <ConfirmationDialog open onClose={() => { setPendingAction(null); setReportDetails(""); }} title={actionLabel} confirmLabel={actionLabel} onConfirm={() => confirmAction()}>{pendingAction === "report" ? <label className="report-details"><b>What happened?</b><Textarea value={reportDetails} onChange={(event) => setReportDetails(event.target.value)} maxLength={2000} placeholder="Describe the issue for the administrator..." /></label> : <p>Are you sure you want to {actionLabel.toLocaleLowerCase()}?</p>}</ConfirmationDialog> : null}
    ...
    <div className="messages-thread">{selected ? <><header><div><h2>{selected.other_user?.name}</h2>{selected.listing_slug ? <Link href={`/listing/${selected.listing_slug}`}>{selected.listing_title}</Link> : null}</div><div><Button variant="ghost" type="button" onClick={() => action(routes.api.conversationMute(selected.id))}>{selected.muted ? t("unmute") : t("mute")}</Button><Button variant="ghost" type="button" onClick={() => action(routes.api.conversationLeave(selected.id))}>{t("leaveConversation")}</Button><Button variant="ghost" type="button" onClick={() => toggleBlock()}>{blocked ? t("unblockUser") : t("blockUser")}</Button><Button variant="ghost" type="button" onClick={() => action(routes.api.conversationReport(selected.id))}>{t("reportConversation")}</Button></div></header><div className="messages-body" ref={messagesBody}>{messagesCursor ? <Button variant="ghost" className="messages-load-older" type="button" title={t("loadOlderMessages")} aria-label={t("loadOlderMessages")} disabled={loadingOlder} onClick={() => loadOlderMessages()}><RefreshCw size={16} className={loadingOlder ? "messages-syncing" : ""} /></Button> : null}{messages.map((message) => { const own = message.sender_id === user?.id; return <article className={own ? "own" : "other"} key={message.id} onClick={() => setReply(message)}><small>{own ? t("yourMessage") : selected.other_user?.name} · {new Date(message.created_at).toLocaleString()}</small>{message.reply_to_id ? <em>{t("replyTo", { id: message.reply_to_id })}</em> : null}<p>{message.body}</p></article>; })}</div><Form onSubmit={send}>{reply ? <button type="button" onClick={() => setReply(null)}>{t("replyTo", { id: reply.id })}</button> : null}<Textarea value={body} onChange={(event) => setBody(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.altKey && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={t("messagePlaceholder")} /><Button variant="primary" type="submit">{t("sendMessage")}</Button></Form></> : <div className="messages-empty">{t("selectConversation")}</div>}</div>
  </section>;
}
