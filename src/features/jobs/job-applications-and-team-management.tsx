...
export function JobApplicationForm({ listingId, ownerUserId }: { listingId: number; ownerUserId?: number }) {
  ...
  return <section className={`${styles.scope} job-apply`}><ConfirmationDialog open={confirming} onClose={() => !saving && setConfirming(false)} title={t("confirmApplication")} cancelLabel={t("cancel")} confirmLabel={saving ? t("loading") : t("submitApplication")} onConfirm={() => submitApplication()} className={styles.applicationConfirm}><p>{t("confirmApplicationDescription")}</p><p><b>{t("resume")}:</b> {profile?.resume_name}</p><p><b>{t("coverLetter")}:</b></p><RichText html={profile?.cover_letter ?? ""} /></ConfirmationDialog><h2>{t("apply")}</h2><p>{t("applyDescription")}</p><Button type="button" variant="secondary" loading={saving} onClick={() => prepareApplication()}>{t("submitApplication")}</Button><FieldErrors error={error} />{message && !hasFieldErrors ? <p role={error ? "alert" : "status"}>{message}{message === t("completeProfileFirst") ? <> <Link href="/my-account/application-profile">{t("manageApplicationProfile")}</Link></> : null}</p> : null}</section>;
}
...
export function TeamManagement() {
  ...
  async function invite(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); const element = event.currentTarget; const form = new FormData(element); setError(null); try { await mutate(routes.api.memberships, "POST", { member_user_id: Number(form.get("member_user_id")), membership_type: form.get("membership_type") }); element.reset(); await load(); } catch (caught) { setError(caught); setMessage(errorMessage(caught, t("operationFailed"))); } }
