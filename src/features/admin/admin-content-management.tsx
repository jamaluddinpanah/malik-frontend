...
import { categoryIconMap, categoryIconOptions } from '@/features/catalog/category-icons';
...
function PageIconPicker({
  ...
  const Preview = Object.hasOwn(categoryIconMap, value) ? categoryIconMap[value] : Tags;
...
export function AdminContentManagement({ view }: { view: View }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (view === 'pages') {
        ...
        const response = await apiClient.request<PageCursorResponse>(`${endpoint.pages}?${query}`);
  ...
  const save = async (files?: Uploads) => {
    ...
    try {
      ...
      if (view === 'settings') {
        ...
        await apiClient.request(id ? routes.api.admin.banner(id) : endpoint.banners, {
          method: 'POST',
          body: form,
        });
      } else {
        const translations = Array.isArray(body.translations)
          ...
          : [];
        await apiClient.request(id ? routes.api.admin.page(id) : endpoint.pages, {
  ...
  const remove = async (id: number) => {
    ...
    try {
      await apiClient.csrfCookie();
      await apiClient.request(
        view === 'pages' ? routes.api.admin.page(id) : routes.api.admin.banner(id),
        { method: 'DELETE' },
      );
  ...
  const title = view === 'settings' ? 'Settings' : view === 'pages' ? 'Pages' : 'Banners';
    ...
    <AdminPageGuard permission={permission[view]}>
      <section className={`admin-content-manager admin-content-manager--${view}`}>
        ...
        <ConfirmationDialog
          ...
          onConfirm={() => (deletingPage ? remove(deletingPage.id) : undefined)}
        >
          <p className="admin-content-delete-message">
            This will delete <b>{String(deletingPage?.slug ?? 'this page')}</b>. This action cannot
            be undone.
          </p>
        </ConfirmationDialog>
        {loading ? (
          ...
          <div
            className={`admin-content-grid${view === 'pages' || view === 'banners' ? ' admin-content-grid--single' : ''}`}
          >
            <div className="admin-content-list">
              {view === 'pages' ? (
                <>
                  <div className="admin-content-pages-list-header">
                    <div>
                      <h2>All pages</h2>
                      <p>
                        {pageTotal} {pageTotal === 1 ? 'item' : 'items'}
                      </p>
                    </div>
                  </div>
                  {items.length ? (
                        ...
                        <table>
                          <thead>
                            <tr>
                              ...
                              {canManage ? (
                                <th>
                                  <span className="admin-content-actions-label">Actions</span>
                                </th>
                              ) : null}
                            </tr>
                          </thead>
                            ...
                            {items.map((item, index) => {
                              ...
                              const locations = Array.isArray(item.navigation_locations)
                                ? item.navigation_locations.filter(
                                    (location): location is keyof typeof navigationLabels =>
                                      typeof location === 'string' && location in navigationLabels,
                                  )
                                : [];
                              ...
                              return (
                                <tr key={item.id}>
                                  ...
                                  <td>
                                    <b>{String(title || item.slug)}</b>
                                    <small>/{String(item.slug)}</small>
                                  </td>
                                  <td>
                                    <span
                                      className={`admin-content-status admin-content-status--${String(item.status ?? 'draft')}`}
                                    >
                                      {status}
                                    </span>
                                  </td>
                                  <td>
                                    {locations.length ? (
                                      <span className="admin-content-placement-chips">
                                        {locations.map((location) => (
                                          <span key={location}>{navigationLabels[location]}</span>
                                        ))}
                                      </span>
                                    ) : (
                                      <span className="admin-content-not-linked">Not linked</span>
                                    )}
                                  </td>
                                  {canManage ? (
                                    <td>
                      ...
                      <nav className="admin-content-cursor-controls" aria-label="Pages pagination">
                        ...
                        <div>
                          ...
                          <Button
                            ...
                            size="sm"
                            variant="ghost"
                            disabled={loading || !pagePagination?.next_cursor}
                            onClick={nextPage}
                          >
                            Next
                          </Button>
                ...
                <>
                  <div className="admin-content-pages-list-header">
                    <div>
                      <h2>All banners</h2>
                      <p>
                        {bannerTotal} {bannerTotal === 1 ? 'item' : 'items'}
                      </p>
                    </div>
                  </div>
                  {items.length ? (
                    <>
                      <div className="admin-content-pages-table admin-content-banners-table">
                        <table>
                          ...
                          <tbody>
                            {items.map((item, index) => (
                              <tr key={item.id}>
                                <td>{bannerStart + index}</td>
                                <td>
                                  <b>Home page</b>
                                </td>
                                <td>
                                  <span
                                    className={`admin-content-status admin-content-status--${item.is_active ? 'active' : 'inactive'}`}
                                  >
                                    {item.is_active ? 'Active' : 'Inactive'}
                                  </span>
                                </td>
                                <td>{String(item.sort_order ?? 0)}</td>
                                <td>{bannerSchedule(item)}</td>
                                {canManage ? (
                                  <td>
                                    <span className="admin-content-row-actions">
                                      <Button
                      ...
                      <nav
                        className="admin-content-cursor-controls"
                        aria-label="Banners pagination"
                      >
                        <span>{`Showing ${bannerStart}-${bannerEnd}`}</span>
                        <div>
                          <Button
                            type="button"
                            size="sm"
                            variant="ghost"
                            disabled={loading || bannerIndex === 0}
                          ...
                          <Button
                            ...
                            size="sm"
                            variant="ghost"
                            disabled={loading || !bannerPagination?.next_cursor}
                            onClick={nextBanner}
                          >
                            Next
                          </Button>
                ...
                items.map((item) => (
                  <article key={item.id}>
                    <div>
                      <b>{String(item.slug ?? item.setting_key ?? item.placement ?? item.id)}</b>
                      <small>
                        {String(
                          item.status ??
                            item.group_name ??
            ...
            {selected && canManage && view !== 'pages' && view !== 'banners' ? (
              ...
              <aside className="admin-content-empty">
                {canManage ? 'Select an item to edit it.' : 'You have view-only access.'}
              </aside>
            )}
          </div>
        )}
...
function Editor({
  ...
  onSave: (files?: Uploads) => Promise<void>;
  onCancel: () => void;
  saving: boolean;
}) {
  ...
  const translations = Array.isArray(item.translations)
    ...
    : [];
  const locations = Array.isArray(item.navigation_locations)
    ? item.navigation_locations.filter((value): value is string => typeof value === 'string')
    : [];
  ...
  const toggleLocation = (location: string, checked: boolean) =>
    set(
      'navigation_locations',
      checked ? [...locations, location] : locations.filter((value) => value !== location),
    );
  ...
  return (
    <Form
      ...
      onSubmit={(event) => {
        ...
        return onSave({ image, mobileImage });
      }}
    >
      <h2>
      ...
      {view === 'settings' ? (
        ...
        <>
          ...
          <label>
            ...
            <Input
              ...
              onChange={(event) =>
                set('navigation_order', event.target.value === '' ? 0 : Number(event.target.value))
              }
            />
          ...
          {(['en', 'fa', 'ps'] as const).map((locale) => {
            const current = translations.find((entry) => entry.locale === locale) ?? {
              locale,
              title: '',
              content: '',
            };
        ...
        <>
          <label>
            Placement
            <Input value="Home page" readOnly required />
