...
type Page = { data: Log[]; next_cursor: string | null; prev_cursor: string | null };
...
export function AdminAuditLogs() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      if (applied) query.set('action', applied);
      if (cursor) query.set('cursor', cursor);
      void apiClient
        .request<{ data: Page; meta: { total: number } }>(`/admin/api/v1/audit-logs?${query}`)
        .then((response) => {
          setPage(response.data);
          setTotal(response.meta.total);
        })
        .catch(() => setError('Audit logs could not be loaded.'))
  ...
  return (
    <AdminPageGuard permission={adminPermissions.auditLogs}>
      <section className={styles.page}>
        <AdminPageHeader
          icon={ScrollText}
          eyebrow="Administration / Audit"
          title="Audit logs"
          description="Review protected user and administrator actions."
        />
        ...
        {loading ? (
          ...
          <>
            ...
            <nav className={styles.pagination} aria-label="Audit log pagination">
              ...
              <button type="button" disabled={!page.prev_cursor} onClick={previous}>
                Previous
              </button>
              <button type="button" disabled={!page.next_cursor} onClick={next}>
                Next
              </button>
            </nav>
          </>
        ...
        {selected ? (
          <div className={styles.modalBackdrop} onClick={() => setSelected(null)}>
            ...
            >
              <header>
                <div>
                  <small>Audit record #{selected.id}</small>
              ...
              <h3>Old values</h3>
              <pre>{pretty(selected.old_values)}</pre>
              <h3>New values</h3>
              <pre>{pretty(selected.new_values)}</pre>
              <h3>Metadata</h3>
              <pre>{pretty(selected.metadata)}</pre>
            </section>
          </div>
        ) : null}
      </section>
