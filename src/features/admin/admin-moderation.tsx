...
function alertCount(summary?: AdminModerationItem['ai_review_summary'] | AiReview | null) {
  return (
    ...
    (summary && 'data' in summary ? summary.data?.flags?.length : 0) ??
    0
  );
}
...
function isAiInProgress(summary?: AdminModerationItem['ai_review_summary'] | AiReview | null) {
  return ['analyzing', 'queued', 'pending', 'running'].includes(
...
function queueRiskLabel(
  summary: AdminModerationItem['ai_review_summary'] | AiReview | null | undefined,
...
function alertSeverity(summary?: AdminModerationItem['ai_review_summary'] | AiReview | null) {
  ...
  if (flags?.some((flag) => typeof flag !== 'string' && /critical|high/i.test(flag.severity ?? '')))
    return 'high';
  if (flags?.some((flag) => typeof flag !== 'string' && /warning|warn/i.test(flag.severity ?? '')))
    return 'warning';
  return 'alert';
}
...
function aiErrorText(error: unknown, t: ReturnType<typeof useTranslations>) {
  ...
  return /api-inference\.huggingface\.co|could not resolve host|huggingface_token is not configured/i.test(
    text,
  )
...
function AiBadge({
  ...
  summary?: AdminModerationItem['ai_review_summary'];
  t: ReturnType<typeof useTranslations>;
  onAlertClick?: () => void;
}) {
  if (!summary)
    return (
      <span className={`${styles.aiBadge} ${styles.aiUnavailable}`}>{t('aiUnavailable')}</span>
    );
  const status = summary.status?.toLowerCase();
  if (status === 'analyzing' || status === 'queued' || status === 'pending' || status === 'running')
    return (
      <span className={`${styles.aiBadge} ${styles.aiAnalyzing}`}>
        <LoaderCircle size={12} /> {t('aiAnalyzing')}
      </span>
    );
  if (status === 'failed' || status === 'error')
    return <span className={`${styles.aiBadge} ${styles.aiFailed}`}>{t('aiFailed')}</span>;
  if (hasAiAlerts(summary)) {
    ...
    return onAlertClick ? (
      <button
        className={`${className} ${styles.aiBadgeButton}`}
        type="button"
        onClick={onAlertClick}
        aria-haspopup="dialog"
        aria-label={t('openAlertDetails', { count: alertCount(summary) })}
      >
        {label}
      </button>
    ) : (
  ...
  const label =
    risk === 'needs_review'
      ? t('aiNeedsReview').replace(/\s+Risk$/i, '')
      : isHighRisk(summary)
        ? t('aiHighRisk')
        : t('aiReviewed');
  return (
    <span className={`${styles.aiBadge} ${isHighRisk(summary) ? styles.aiHigh : ''}`}>{label}</span>
  );
}

function UnrelatedImagesBadge({
  summary,
  t,
}: {
  summary?: AdminModerationItem['ai_review_summary'];
...
export function AdminModerationQueue() {
  ...
  async function load(requestedCursor = cursor) {
    ...
    setError(null);
    try {
      const response = await repository.moderationQueue({
        q: applied,
  ...
  const pollAnalysis = useEffectEvent(async () => {
    ...
    const results = await Promise.allSettled(
      analysisItems.map(async (item) => ({
        id: item.id,
        summary: queueSummary(await repository.moderationAiReview(item.id)),
      })),
    );
    ...
    const update = (item: AdminModerationItem) =>
      summaries.has(item.id) ? { ...item, ai_review_summary: summaries.get(item.id) } : item;
    setAnalysisItems((current) => current?.map(update) ?? current);
    setPage((current) => (current ? { ...current, data: current.data.map(update) } : current));
  });
  const analysisStatusKey = analysisItems
    ?.map((item) => `${item.id}:${item.ai_review_summary?.status ?? ''}`)
    .join(',');
  useEffect(() => {
    if (!analysisItems?.some((item) => isAiInProgress(item.ai_review_summary))) return;
  ...
  async function analyzeCurrentPage() {
    ...
    try {
      ...
      const queuedItems = page.data.map((item) => ({
        ...item,
        ai_review_summary: { ...item.ai_review_summary, status: 'queued' },
      }));
  ...
  async function approve() {
    ...
    setApproving(true);
    try {
      await repository.decideModeration(approveTarget.id, {
        decision: 'approved',
        ...(isSuperadmin ? { duration_days: Number(approveDuration) } : {}),
      });
      setPage((current) =>
        current
          ? { ...current, data: current.data.filter((item) => item.id !== approveTarget.id) }
          : current,
      );
  ...
  const selectedAlertCount = selectedItems.filter(
    (item) => hasAiAlerts(item.ai_review_summary) || isHighRisk(item.ai_review_summary),
  ).length;
  const allVisibleSelected =
    Boolean(page?.data.length) && page!.data.every((item) => selectedIds.includes(item.id));
  ...
  async function bulkApprove() {
    if (
      !selectedIds.length ||
      (selectedAlertCount > 0 && (!isSuperadmin || !bulkAiOverride || !bulkAiOverrideReason.trim()))
    )
      return;
    ...
    try {
      const result = await repository.bulkApproveModeration({
        ...
        ...(isSuperadmin && selectedAlertCount > 0
          ? { ai_override_reason: bulkAiOverrideReason.trim() }
          : {}),
      });
      setBulkNotice(
        t('bulkApprovalResult', {
          approved: result.approved_listing_ids.length,
  ...
  function openAlertDetails(item: AdminModerationItem) {
    ...
    void (async () => {
      try {
        ...
        const hasMediaAlert = alertFlags(review).some(
          (alert) =>
            typeof alert !== 'string' && alert.media_id !== null && alert.media_id !== undefined,
        );
        if (hasMediaAlert) {
    ...
    <AdminPageGuard permission={adminPermissions.listingsModerate}>
      <section className={styles.page}>
        <AdminPageHeader
          ...
          actions={<span className={styles.statusBadge}>{t('pendingLabel')}</span>}
        />
        <Form
          className={`${styles.toolbar} ${styles.toolbarPanel}`}
          onSubmit={(event) => {
        ...
        >
          ...
          />
          <Select
            aria-label={t('aiFilter')}
            value={aiRiskFilter}
            onChange={(event) => {
              setOffset(0);
              setCursor(null);
              setAiRiskFilter(event.target.value);
            }}
          >
            <option value="">{t('aiAll')}</option>
            <option value="high">{t('aiHighRisk')}</option>
          ...
          <button
            ...
            type="button"
            disabled={!page?.data.length || analyzing}
            onClick={() => setAnalyzeOpen(true)}
          >
            {analyzing ? t('aiAnalyzing') : t('analyzeCurrentPage')}
          </button>
        </Form>
        {bulkNotice ? (
          <Toast
        ...
        {loading ? (
          ...
          <div className={`${styles.state} ${styles.error}`}>
            <p>{errorText(error, t('loadError'))}</p>
          ...
          <>
            {selectedIds.length ? (
              <div className={styles.bulkActions}>
                <button type="button" onClick={() => setBulkApproveOpen(true)}>
                  ...
                  <tr>
                    <th>
                      <input
                        ...
                        aria-label={t('selectAllCurrentPage')}
                      />
                    </th>
                    <th>Count</th>
                    <th>{t('listing')}</th>
                    ...
                    <tr key={item.id}>
                      <td>
                        <input
                          ...
                          aria-label={t('selectListing', { listing: item.title })}
                        />
                      </td>
                      <td>{offset + index + 1}</td>
                      <td>
                        <Link href={`/admin/moderation/${item.id}`}>{item.title}</Link>
                        <small>{item.listing_number}</small>
                      </td>
                      <td>{item.owner?.name ?? '-'}</td>
                      <td>{item.category?.slug ?? '-'}</td>
                      <td>
                        <div className={styles.aiQueueIndicators}>
                          <AiBadge
                            summary={item.ai_review_summary}
                            t={t}
                            onAlertClick={() => openAlertDetails(item)}
                          />
                          <UnrelatedImagesBadge summary={item.ai_review_summary} t={t} />
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        <span className={styles.queueActions}>
                          <Link href={`/admin/moderation/${item.id}`}>{t('review')}</Link>
                          <button
                            type="button"
                            disabled={
                              isHighRisk(item.ai_review_summary) ||
                              hasAiAlerts(item.ai_review_summary)
                            }
                            title={
                              isHighRisk(item.ai_review_summary) ||
                              hasAiAlerts(item.ai_review_summary)
            ...
            <nav className={styles.pagination} aria-label={t('pagination')}>
              ...
              <button
                ...
                onClick={() => {
                  setOffset((value) => value + page.data.length);
      ...
      <ConfirmationDialog
        ...
        confirmLabel={approving ? t('saving') : t('approved')}
        cancelLabel={t('cancel')}
        onConfirm={() => approve()}
      >
        <p>{t('confirmDescription')}</p>
        {isSuperadmin ? (
          <label className={`${styles.duration} ${styles.approvalDuration}`}>
            ...
            >
              ...
              <option value="365">{t('duration365')}</option>
            </Select>
          </label>
        ) : null}
      </ConfirmationDialog>
      <ConfirmationDialog
        ...
        onClose={() => !bulkApproving && setBulkApproveOpen(false)}
        title={t('bulkApproveTitle')}
        confirmLabel={bulkApproving ? t('saving') : t('approved')}
        cancelLabel={t('cancel')}
      ...
      >
        ...
        {selectedAlertCount ? (
          <div className={styles.highRiskWarning}>
            <ShieldAlert size={18} />
            <div>
              <strong>{t('bulkAlertsWarning', { count: selectedAlertCount })}</strong>
              <p>{isSuperadmin ? t('aiOverrideInstructions') : t('bulkAlertsWillBeSkipped')}</p>
              {isSuperadmin ? (
                <>
                  <label className={styles.overrideCheck}>
                    <input
                      type="checkbox"
                      checked={bulkAiOverride}
                      onChange={(event) => setBulkAiOverride(event.target.checked)}
                  ...
                  {bulkAiOverride ? (
                    <Input
                      required
                      value={bulkAiOverrideReason}
                      onChange={(event) => setBulkAiOverrideReason(event.target.value)}
                      placeholder={t('aiOverrideReason')}
                      disabled={bulkApproving}
                    />
                  ) : null}
                </>
      ...
      <ConfirmationDialog
        open={analyzeOpen}
        onClose={() => !analyzing && setAnalyzeOpen(false)}
        title={t('analyzeConfirmTitle')}
        confirmLabel={analyzing ? t('aiAnalyzing') : t('analyzeCurrentPage')}
        cancelLabel={t('cancel')}
        onConfirm={() => analyzeCurrentPage()}
      >
        <p>{t('analyzeConfirmDescription', { count: page?.data.length ?? 0 })}</p>
      </ConfirmationDialog>
      <Dialog
        ...
        footer={
          <button
            className={styles.analysisClose}
            type="button"
            onClick={() => setAnalysisResultsOpen(false)}
          >
            {t('cancel')}
          </button>
        }
        ...
        {analysisItems ? (
          <div className={styles.analysisResults}>
            <p className={styles.analysisIntro}>
              {t('analysisResultsDescription', { count: analysisItems.length })}
            </p>
            <div className={styles.analysisCounts}>
              <span>
                {t('analysisClearCount', {
                  count: analysisItems.filter(
                    (item) =>
                      item.ai_review_summary?.status === 'completed' &&
                      !hasAiAlerts(item.ai_review_summary),
                  ).length,
                })}
              </span>
              <span>
                {t('analysisAlertCount', {
                  count: analysisItems.filter((item) => hasAiAlerts(item.ai_review_summary)).length,
                })}
              </span>
            </div>
              ...
              return (
                <article className={styles.analysisItem} key={item.id}>
                  <div className={styles.analysisItemHeader}>
                    ...
                    <AiBadge summary={summary} t={t} />
                  </div>
                  <p>
                    {t('analysisCurrentState')}: <strong>{queueStatusLabel(summary, t)}</strong>
                  </p>
                  <p>
                    {t('analysisSafetyRisk')}: <strong>{queueRiskLabel(summary, t)}</strong>
                  </p>
                  {flags.length ? (
                    <ul>
                      {flags.map((flag, index) => (
                        <li key={`${findingText(flag)}-${index}`}>{findingText(flag)}</li>
                      ))}
                    </ul>
                  ) : summary?.status?.toLowerCase() === 'completed' ? (
                    <p className={styles.analysisClear}>{t('analysisNoFlags')}</p>
                  ) : summary?.status?.toLowerCase() === 'failed' ||
                    summary?.status?.toLowerCase() === 'error' ? (
                    <p className={styles.analysisError}>
                      {aiErrorText(summary.error, t) || t('analysisFailed')}
                    </p>
                  ) : (
                    <p className={styles.analysisWaiting}>{t('analysisWaiting')}</p>
                  )}
                </article>
              );
            })}
      ...
      <Dialog
        ...
        title={t('alertDetailsTitle', { listing: alertTarget?.title ?? '' })}
        className={styles.alertDetailsDialog}
        footer={
          <button
            className={styles.analysisClose}
            type="button"
      ...
      >
        {alertLoading ? (
          <div className={styles.alertDetailsLoading}>
            <LoaderCircle /> {t('alertDetailsLoading')}
          </div>
        ) : alertError ? (
          <p className={styles.aiError}>{errorText(alertError, t('alertDetailsLoadError'))}</p>
        ) : alertReview ? (
          <div className={styles.aiAlerts}>
            ...
            {alertFlags(alertReview).map((alert, index) => {
              const media =
                typeof alert === 'string'
                  ? undefined
                  : alertMedia?.find((item) => item.id === alert.media_id);
              const severity =
                typeof alert === 'string' ? 'alert' : alertSeverity({ flags: [alert] });
              const category =
                typeof alert === 'string' ? null : (alert.category ?? alert.detected_label);
              return (
                <article
                  className={`${styles.aiAlert} ${severity === 'high' ? styles.aiAlertHigh : severity === 'warning' ? styles.aiAlertWarning : ''}`}
                  key={`${findingText(alert)}-${index}`}
                >
                  ...
                  <div>
                    <span className={styles.aiAlertSeverity}>
                      {t(`aiSeverity${severity[0].toUpperCase()}${severity.slice(1)}`)}
                    </span>
                    <p>{findingText(alert)}</p>
                    {typeof alert !== 'string' ? (
                      <small>
                        {[
                          category ? t('detectedCategory', { category }) : null,
                          alert.root_type ? t('rootType', { type: alert.root_type }) : null,
                          alert.confidence !== null && alert.confidence !== undefined
                            ? t('confidence', { value: Math.round(Number(alert.confidence) * 100) })
                            : null,
                          media ? t('aiAlertImage') : null,
                        ]
                          .filter(Boolean)
                          .join(' · ')}
                      </small>
...
export function AdminModerationReview() {
  ...
  function submit(event: React.FormEvent) {
    ...
    if (
      decision === 'approved' &&
      (isHighRisk(aiReview ?? listing?.ai_review_summary) ||
        hasAiAlerts(aiReview) ||
        hasAiAlerts(listing?.ai_review_summary)) &&
      (!isSuperadmin || !aiOverride || !aiOverrideReason.trim())
    )
      return;
    setConfirmOpen(true);
  }
  ...
  async function refreshAiReview(analyze = false) {
    ...
    try {
      ...
      if (failure instanceof ApiError && [404, 403, 422, 503].includes(failure.status))
        setAiReviewUnavailable(true);
  ...
  async function confirmDecision() {
    ...
    try {
      setListing(
        await repository.decideModeration(params.id, {
          decision,
          reason_code: reason || undefined,
          notes: notes || undefined,
          ...(isSuperadmin && decision === 'approved'
            ? { duration_days: Number(durationDays) }
            : {}),
          ...(isSuperadmin && decision === 'approved' && aiOverride
            ? { ai_override_reason: aiOverrideReason.trim() }
            : {}),
        }),
      );
      setNotice(t('saved'));
  ...
  const attributeValue = (item: NonNullable<AdminModerationDetail['values']>[number]) => {
    const condition =
      item.attribute?.code === 'vehicle_body_condition'
        ? vehicleConditionValue(item.json_value)
        : null;
    ...
    return displayValue(
      item.string_value ??
        ...
        item.decimal_value ??
        item.boolean_value ??
        (item.json_value ? JSON.stringify(item.json_value) : null),
    );
  };
  const translation = (locale: (typeof locales)[number]) =>
  ...
  const mediaForReview = (review: AiImageReview) =>
    review.media?.thumbnail_url ??
    review.thumbnail_url ??
    review.media?.url ??
    review.url ??
    listing.media?.find((media) => media.id === (review.media_id ?? review.image_id))
      ?.thumbnail_url ??
    listing.media?.find((media) => media.id === (review.media_id ?? review.image_id))?.url;

  return (
    <AdminPageGuard permission={adminPermissions.listingsModerate}>
      <section className={styles.page}>
        <Link className={styles.backLink} href="/admin/moderation">
        ...
        <AdminPageHeader
          ...
          eyebrow={t('sectionLabel')}
          title={listing.title}
          description={`${listing.listing_number} · ${listing.moderation_status}`}
          actions={
            <span className={`${styles.statusBadge} ${styles[listing.moderation_status] ?? ''}`}>
        ...
        <div className={styles.review}>
          <div className={styles.details}>
            <section className={styles.panel}>
              ...
              <div className={styles.fields}>
                <div className={styles.field}>
                  ...
                  <span>{displayValue(listing.owner?.name)}</span>
                </div>
                <div className={styles.field}>
                  <strong>{t('category')}</strong>
                  <span>
                    {listing.category_path?.map((item) => item.slug).join(' / ') ??
                      listing.category?.slug ??
                      '-'}
                  </span>
                </div>
                <div className={styles.field}>
                  <strong>{t('listingLanguage')}</strong>
                  <span>{listing.language_code}</span>
                </div>
                <div className={styles.field}>
                  <strong>{t('condition')}</strong>
                  <span>{displayValue(listing.condition)}</span>
                </div>
              </div>
            </section>
            <section className={styles.panel}>
              ...
              <div className={styles.translations}>
                {locales.map((locale) => (
                  <article className={styles.translation} key={locale}>
                    <h3>{t(`language${locale.toUpperCase()}`)}</h3>
                    <div>
                      <strong>{t('titleField')}</strong>
                      <p>{translation(locale)?.title ?? '-'}</p>
                    </div>
                    <div>
                      <strong>{t('descriptionField')}</strong>
                      {translation(locale)?.description ? (
                        <RichText html={translation(locale)!.description} />
                      ) : (
                        <p>-</p>
                      )}
                    </div>
            ...
            <section className={styles.panel}>
              ...
              <div className={styles.fields}>
                <div className={styles.field}>
                  ...
                  <span>{displayValue(listing.price)}</span>
                </div>
                <div className={styles.field}>
                  <strong>{t('priceType')}</strong>
                  <span>{displayValue(listing.price_type)}</span>
                </div>
                <div className={styles.field}>
                  <strong>{t('currency')}</strong>
                  <span>{displayValue(listing.currency?.code)}</span>
                </div>
                <div className={styles.field}>
                ...
                <div className={styles.field}>
                  <strong>{t('salaryPeriod')}</strong>
                  <span>{displayValue(listing.salary_period)}</span>
                </div>
              </div>
            </section>
            <section className={styles.panel}>
            ...
            <section className={styles.panel}>
              ...
              <div className={styles.fields}>
                <div className={styles.field}>
                  ...
                  <span>{displayValue(listing.contact_name)}</span>
                </div>
                <div className={styles.field}>
                  <strong>{t('contactPhone')}</strong>
                  <span>{displayValue(listing.contact_phone)}</span>
                </div>
                <div className={styles.field}>
                  <strong>{t('contactEmail')}</strong>
                  <span>{displayValue(listing.contact_email)}</span>
                </div>
              </div>
            </section>
            <section className={styles.panel}>
              ...
              <div className={styles.fields}>
                <div className={`${styles.field} ${styles.wide}`}>
                  ...
                  <span>{displayValue(listing.address)}</span>
                </div>
                <div className={styles.field}>
                  <strong>{t('administrativeArea')}</strong>
                  <span>{displayValue(listing.administrative_area_id)}</span>
                </div>
                <div className={styles.field}>
            ...
            <section className={styles.panel}>
              <h2>{t('attributes')}</h2>
              {listing.values?.length ? (
                <div className={styles.fields}>
                  {listing.values.map((item) => (
                    <div
                      className={`${styles.field} ${item.attribute?.code === 'vehicle_body_condition' ? styles.wide : ''}`}
                      key={item.attribute_id}
                    >
                      <strong>{item.attribute?.code ?? item.attribute_id}</strong>
                      <span>{attributeValue(item)}</span>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <section className={styles.panel}>
              <div className={styles.aiHeader}>
                ...
                <Button
                  ...
                  disabled={
                    aiRefreshing || aiReview?.status === 'queued' || aiReview?.status === 'running'
                  }
                >
                  <RefreshCw size={14} />{' '}
                  {aiRefreshing || aiReview?.status === 'queued' || aiReview?.status === 'running'
              ...
              {aiReviewUnavailable ? (
                <p className={styles.panelMessage}>{t('aiUnavailableDescription')}</p>
              ) : aiReview ? (
                <div className={styles.aiPanelBody}>
                  <div className={styles.aiSummary}>
                    <AiBadge summary={aiReview} t={t} />
                    {(aiReview.analyzed_at ?? aiReview.created_at) ? (
                      <small>
                        {t('analyzedAt', {
                          date: new Date(
                            aiReview.analyzed_at ?? aiReview.created_at ?? '',
                  ...
                  {aiReview.provider || aiReview.model ? (
                    <p className={styles.aiMeta}>
                      {[aiReview.provider, aiReview.model].filter(Boolean).join(' / ')}
                    </p>
                  ) : null}
                  {aiAlerts.length ? (
                    <div className={styles.aiAlerts}>
                      <strong>{t('aiModerationAlerts')}</strong>
                      <p>{t('aiAlertsAdvisory')}</p>
                      {aiAlerts.map((alert, index) => {
                        const media =
                          typeof alert === 'string'
                            ? undefined
                            : listing.media?.find((item) => item.id === alert.media_id);
                        const severity =
                          typeof alert === 'string' ? 'alert' : alertSeverity({ flags: [alert] });
                        return (
                          <article
                            ...
                            <div>
                              ...
                              {typeof alert !== 'string' ? (
                                <small>
                                  {[
                                    alert.code,
                                    alert.category ?? alert.root_type,
                                    alert.detected_label,
                                    alert.confidence !== null && alert.confidence !== undefined
                                      ? t('confidence', {
                                          value: Math.round(Number(alert.confidence) * 100),
                                        })
                                      : null,
                                    media ? t('aiAlertImage') : null,
                                  ]
                  ...
                  {textFlags.length ? (
                    <div className={styles.aiFindings}>
                      <strong>{t('textFlags')}</strong>
                      {textFlags.map((flag, index) => (
                        <div key={`${findingText(flag)}-${index}`}>
                          <span>{findingText(flag)}</span>
                          {typeof flag !== 'string' &&
                          (flag.confidence ?? flag.score) !== undefined ? (
                            <small>
                              {t('confidence', {
                                value: Math.round(Number(flag.confidence ?? flag.score) * 100),
                              })}
                            </small>
                          ) : null}
                        </div>
                  ...
                  ) : !aiAlerts.length && aiReview.status === 'completed' ? (
                    <p className={styles.panelMessage}>{t('noAiFlags')}</p>
                  ) : null}
                  {imageReviews.length ? (
                    <div className={styles.aiFindings}>
                      <strong>{t('imageFindings')}</strong>
                      {imageReviews.map((image, index) => (
                        <article
                          className={`${styles.aiImageFinding} ${styles[`categoryMatch${categoryMatch(image)}`] ?? ''}`}
                          key={`${image.media_id ?? image.image_id ?? 'image'}-${index}`}
                        >
                          {mediaForReview(image) ? (
                            <img src={mediaForReview(image)} alt="" />
                          ) : null}
                          <div className={styles.aiImageDetails}>
                            {image.category_match ? (
                              <div className={styles.categoryMatch}>
                                <span
                                  className={`${styles.categoryMatchBadge} ${styles[`categoryMatchBadge${categoryMatch(image)}`] ?? ''}`}
                                >
                                  {t(`categoryMatch${categoryMatch(image)}`)}
                                </span>
                                {image.category_label ? (
                                  <span>
                                    {t('detectedCategory', { category: image.category_label })}
                                  </span>
                                ) : null}
                                {image.category_confidence !== null &&
                                image.category_confidence !== undefined ? (
                                  <small>
                                    {t('categoryConfidence', {
                                      value: Math.round(Number(image.category_confidence) * 100),
                                    })}
                                  </small>
                                ) : null}
                              </div>
                            ) : null}
                            {(image.findings ?? image.flags ?? []).map((flag, findingIndex) => (
                              <p key={`${findingText(flag)}-${findingIndex}`}>
                                {findingText(flag)}
                                {typeof flag !== 'string' &&
                                (flag.confidence ?? flag.score) !== undefined ? (
                                  <small>
                                    {t('confidence', {
                                      value: Math.round(
                                        Number(flag.confidence ?? flag.score) * 100,
                                      ),
                                    })}
                                  </small>
                                ) : null}
                  ...
                  {aiReview.error || aiReview.errors ? (
                    <p className={styles.aiError}>
                      {aiErrorText(aiReview.error ?? aiReview.errors, t)}
                    </p>
                  ) : null}
                </div>
              ) : (
                <p className={styles.panelMessage}>{t('aiNotAnalyzed')}</p>
              )}
            </section>
            <section className={styles.panel}>
              <h2>{t('decision')}</h2>
              {listing.moderation_status === 'pending' ? (
                <Form className={styles.actions} onSubmit={submit}>
                  <Select value={decision} onChange={(event) => setDecision(event.target.value)}>
                    ...
                    <option value="requires_changes">{t('requiresChanges')}</option>
                    <option value="spam">{t('spam')}</option>
                    <option value="duplicate">{t('duplicate')}</option>
                  </Select>
                  {highRisk && decision === 'approved' ? (
                    <div className={styles.highRiskWarning}>
                      <ShieldAlert size={18} />
                      <div>
                        <strong>{t('aiHighRiskWarning')}</strong>
                        <p>{isSuperadmin ? t('aiOverrideInstructions') : t('aiApprovalBlocked')}</p>
                        {isSuperadmin ? (
                          <>
                            <label className={styles.overrideCheck}>
                              <input
                  ...
                  />
                  {fieldErrors.reason_code?.map((message) => (
                    <small className={styles.error} key={message}>
                      {message}
                    </small>
                  ))}
                  {fieldErrors.ai_override_reason?.map((message) => (
                    <small className={styles.error} key={message}>
                      {message}
                    </small>
                  ))}
                  {error ? (
                    <p className={styles.error}>{errorText(error, t('saveError'))}</p>
                  ) : null}
                  ...
                  >
                    {saving ? t('saving') : t('saveDecision')}
                  </Button>
                </Form>
              ) : (
                <p className={styles.panelMessage}>{t('alreadyDecided')}</p>
              )}
            </section>
            <section className={styles.panel}>
              <h2>{t('history')}</h2>
              {history.length ? (
                <div className={styles.history}>
                  {history.map((item, index) => (
                    <article key={String(item.id ?? index)}>
                      <strong>{displayValue(item.decision as string | null)}</strong>
                      {item.reason_code ? <span>{String(item.reason_code)}</span> : null}
                      {item.notes ? <span>{String(item.notes)}</span> : null}
                      <small>
                        {displayValue(item.moderator_name as string | null)} ·{' '}
                ...
                <p className={styles.panelMessage}>{t('noHistory')}</p>
              )}
            </section>
          </aside>
        </div>
