...
type NumberKey =
  'listings.default_duration' | 'listings.view_deduplication_minutes' | 'media.maximum_upload_size';
...
const definitions: {
  ...
  {
    group: 'Listings',
    key: 'listings.default_duration',
    title: 'Default listing duration',
    description: 'Listing expiry duration.',
    unit: 'days',
    defaultValue: '30',
  },
  {
    group: 'Listings',
    key: 'listings.view_deduplication_minutes',
    title: 'Repeat-view window',
    description: 'Per-viewer repeat-view analytics window.',
    unit: 'minutes',
    defaultValue: '60',
  },
  {
    group: 'Privacy',
    key: 'contact.require_login_for_phone',
    title: 'Sign-in required for phone reveal',
    description: 'Require sign-in before phone reveal.',
    defaultValue: true,
  },
  {
    group: 'Media',
    key: 'media.maximum_upload_size',
    title: 'Maximum upload size',
    description: 'Maximum banner/listing upload size.',
    unit: 'KB',
    defaultValue: '10240',
  },
];
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      do {
        ...
        const response = await apiClient.request<{ data: LaravelCursorPagination<Setting> }>(
          `${routes.api.admin.settings}?${query}`,
        );
      ...
      .then((settings) => {
        ...
        const stored = new Map(
          settings.map((setting) => [
            `${setting.group_name}.${setting.setting_key}`,
            setting.value,
          ]),
        );
        setValues(
    ...
    <AdminPageGuard permission={adminPermissions.settings}>
      <section className={styles.page}>
        ...
        <AdminPageHeader
          ...
          title="Settings"
          description="Control marketplace defaults, privacy, and media limits."
        />
        {loading ? (
          ...
          ['Listings', 'Privacy', 'Media'].map((group) => (
            <section className={styles.group} key={group}>
              ...
              <div className={styles.card}>
                {definitions
                  ...
                  .map((definition) => (
                    <div className={styles.row} key={definition.key}>
                      ...
                      <div className={styles.control}>
                        {definition.unit ? (
                          <label>
                            ...
                            <div className={styles.number}>
                              <Input
                                ...
                                step="1"
                                inputMode="numeric"
                                value={values[definition.key] as string}
                                onChange={(event) => change(definition.key, event.target.value)}
                                disabled={!mayManage}
                                aria-invalid={Boolean(fieldErrors[definition.key])}
                                aria-describedby={
                                  fieldErrors[definition.key]
                          ...
                          <Switch
                            ...
                            description={values[definition.key] ? 'Enabled' : 'Disabled'}
                            checked={values[definition.key] as boolean}
                            onCheckedChange={(checked) => change(definition.key, checked)}
                            disabled={!mayManage}
                          />
                        )}
                        {fieldErrors[definition.key] ? (
                          <small className={styles.error} id={`${definition.key}-error`}>
                            {fieldErrors[definition.key]}
                          </small>
                        ) : null}
                      </div>
                      {mayManage ? (
                        <Button
                          size="sm"
                          onClick={() => save(definition.key)}
                          disabled={saving === definition.key}
                        >
                          {saving === definition.key ? 'Saving...' : 'Save'}
                        </Button>
                      ) : null}
                    </div>
                  ))}
