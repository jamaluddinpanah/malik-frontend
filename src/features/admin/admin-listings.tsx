...
function attributeValue(item: NonNullable<AdminModerationDetail['values']>[number]) {
  return (
    ...
    item.decimal_value ??
    item.boolean_value ??
    (item.json_value ? JSON.stringify(item.json_value) : '-')
  );
}
  ...
  return (
    <AdminPageGuard permission={adminPermissions.listingsViewAll}>
      <section className={styles.page}>
        ...
        >
          ...
          >
            ...
            {(
              ['draft', 'pending', 'published', 'paused', 'sold', 'rejected', 'archived'] as const
            ).map((item) => (
          ...
          <>
            <div className={styles.table}>
              <table>
                ...
                <tbody>
                  {page.data.map((listing, index) => (
                    <tr key={listing.id}>
                      <td>{offset + index + 1}</td>
                      <td>
                        <div className={styles.listingCell}>
                          {listing.media?.[0]?.thumbnail_url || listing.media?.[0]?.url ? (
                            <img
                          ...
                          <span className={styles.listingInfo}>
                            <b>{listing.title}</b>
                            <small>{listing.listing_number}</small>
                          </span>
                        </div>
                      </td>
                      <td>{listing.owner?.name ?? '-'}</td>
                      <td>{listing.category?.slug ?? '-'}</td>
                      <td>
                        <span className={`${styles.status} ${styles[`status${listing.status}`]}`}>
                          {listing.status}
                        </span>
                      </td>
                      <td>
                        {canFeature ? (
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                const featured = await repository.featureListing(
                                  listing.id,
                                  !listing.is_featured,
                                );
                                setPage((current) =>
                          ...
                          >
                            {listing.is_featured ? t('unfeature') : t('feature')}
                          </Button>
                        ) : listing.is_featured ? (
                          t('yes')
                        ) : (
            ...
            <nav className={styles.pagination} aria-label={t('pagination')}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setApplied((current) => ({ ...current, cursor: page.prev_cursor }));
                }}
              >
                {t('previous')}
              </button>
      ...
      <Dialog
        ...
        title={selected?.title ?? t('viewDetails')}
        className={styles.detailsDialog}
      >
        {!detail && !detailError ? (
          <p>{t('loadingDetails')}</p>
        ) : detailError ? (
          <p className={styles.error}>{message(detailError, t('loadDetailsError'))}</p>
        ) : detail ? (
          <div className={styles.detailContent}>
            <section className={styles.detailSection}>
              <h3>{t('listingDetails')}</h3>
              <div className={styles.detailGrid}>
                <div>
                  <strong>{t('seller')}</strong>
                  <span>{displayValue(detail.owner?.name)}</span>
                </div>
                <div>
                  <strong>{t('category')}</strong>
                  <span>{displayValue(detail.category?.slug)}</span>
                </div>
                <div>
                  <strong>{t('status')}</strong>
                  <span>{displayValue(detail.status)}</span>
                </div>
                <div>
                  <strong>{t('condition')}</strong>
                  <span>{displayValue(detail.condition)}</span>
                </div>
              </div>
            </section>
            <section className={styles.detailSection}>
              <h3>{t('submittedTranslations')}</h3>
              <div className={styles.translations}>
                {locales.map((locale) => {
                  const translation = detail.translations.find((item) => item.locale === locale);
                  return (
                    <article className={styles.translation} key={locale}>
                      <h4>{t(`language${locale.toUpperCase()}`)}</h4>
                      <div>
                      ...
                      <div>
                        ...
                        {translation?.description ? (
                          <RichText html={translation.description} />
                        ) : (
                          <p>-</p>
                        )}
                      </div>
            ...
            <section className={styles.detailSection}>
              ...
              <div className={styles.detailGrid}>
                <div>
                  ...
                  <span>{displayValue(detail.price)}</span>
                </div>
                <div>
                  <strong>{t('priceType')}</strong>
                  <span>{displayValue(detail.price_type)}</span>
                </div>
                <div>
                  <strong>{t('currency')}</strong>
                  <span>{displayValue(detail.currency?.code)}</span>
                </div>
                <div>
                  <strong>{t('priceRange')}</strong>
                  <span>{`${displayValue(detail.minimum_price)} - ${displayValue(detail.maximum_price)}`}</span>
                </div>
                <div>
                  <strong>{t('salaryPeriod')}</strong>
                  <span>{displayValue(detail.salary_period)}</span>
                </div>
              </div>
            </section>
            <section className={styles.detailSection}>
              <h3>{t('listingFlags')}</h3>
              <div className={styles.detailGrid}>
                {(
                  [
                    ['isNegotiable', detail.is_negotiable],
                    ['isUrgent', detail.is_urgent],
                    ['isPhoneVisible', detail.is_phone_visible],
                    ['allowMessages', detail.allow_messages],
                  ] as const
                ).map(([label, enabled]) => (
                  <div key={label}>
            ...
            <section className={styles.detailSection}>
              ...
              <div className={styles.detailGrid}>
                <div>
                  ...
                  <span>{displayValue(detail.contact_name)}</span>
                </div>
                <div>
                  <strong>{t('contactPhone')}</strong>
                  <span>{displayValue(detail.contact_phone)}</span>
                </div>
                <div>
                  <strong>{t('contactEmail')}</strong>
                  <span>{displayValue(detail.contact_email)}</span>
                </div>
              </div>
            </section>
            <section className={styles.detailSection}>
              ...
              <div className={styles.detailGrid}>
                <div className={styles.detailWide}>
                  ...
                  <span>{displayValue(detail.address)}</span>
                </div>
                <div>
                  <strong>{t('administrativeArea')}</strong>
                  <span>{displayValue(detail.administrative_area_id)}</span>
                </div>
                <div>
                  ...
                  {detail.values.map((item) => {
                    const condition =
                      item.attribute?.code === 'vehicle_body_condition'
                        ? vehicleCondition(item.json_value)
                        : null;
                    return (
                      <div
                        key={item.attribute_id}
                        className={condition ? styles.attributeWide : ''}
                      >
                        <strong>{item.attribute?.code ?? item.attribute_id}</strong>
                        {condition ? (
                          <VehicleBodyConditionMap value={condition} readOnly />
                        ) : (
