...
type Report = {
  ...
  messaging_disabled_at?: string | null;
};
...
export function AdminReports() {
  ...
  async function load() {
    ...
    try {
      ...
      const response = await apiClient.request<{ data: Page; meta: { total: number } }>(
        `${routes.api.admin.reports}?${query}`,
      );
  ...
  async function decide(
    id: number,
    nextStatus: 'reviewing' | 'resolved' | 'rejected' | 'escalated',
    messaging?: 'disable' | 'enable',
  ) {
    try {
      await apiClient.csrfCookie();
      await apiClient.request(routes.api.admin.report(id), {
        method: 'PATCH',
        body: {
          status: nextStatus,
          disable_messaging: messaging === 'disable',
          enable_messaging: messaging === 'enable',
        },
      });
      await load();
      if (messaging)
        setNotice({
          title: messaging === 'disable' ? 'Messaging disabled' : 'Messaging enabled',
          message:
            messaging === 'disable'
              ? 'The reported user can no longer start or send messages.'
      ...
      setNotice({
        title: 'Action failed',
        message: reason instanceof ApiError ? reason.message : 'The report could not be updated.',
        tone: 'danger',
      });
    } finally {
      setSelected(null);
  ...
  return (
    <AdminPageGuard permission={adminPermissions.messagesModerate}>
      <section className={styles.page}>
        ...
        {selected ? (
          <ConfirmationDialog
            ...
            onConfirm={() => decide(selected.id, 'resolved', isEnable ? 'enable' : 'disable')}
          >
            <p>
              {isEnable
                ? "Restore this user's ability to start and send messages?"
                : 'Resolve this report and disable messaging for the reported user?'}
            </p>
          </ConfirmationDialog>
        ) : null}
        <AdminPageHeader
          ...
          title="Reports"
          description="Review reported listings, users, messages, and conversations."
        />
        <Form
        ...
        {loading ? (
          ...
          <div className={styles.state}>No reports found.</div>
        ) : (
          <>
                    ...
                    <tr key={report.id}>
                      ...
                      <td>
                        ...
                        <button type="button" onClick={() => setSelected(report)}>
                          {report.messaging_disabled_at ? 'Enable messaging' : 'Disable messaging'}
                        </button>
                      </td>
                    </tr>
                  ))}
            ...
            <nav className={styles.pagination} aria-label="Reports pagination">
              ...
              <button
                ...
                onClick={() => {
                  setOffset((value) => value + reports.length);
